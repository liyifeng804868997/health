...
public class OrderServiceImpl implements OrderService {
    ...
    public Result order(Map map) throws Exception {
        ...
        //3.检查用户是否重复预约（同一个用户在用一天预约了同一个套餐），如果是重复预约则无法完成再次预约
        ...
        if (member != null) {
            ...
            if (name != null && name.length() > 0 || idCard2!=null&&idCard2.length()>0) {
                if (!name.equals(name1)) {
                    //说明用户体检人姓名与会员名不一致
                    return new Result(false, MessageConstant.NAME_ORDERED);
                }
                if (!idCard.equals(idCard2)) {
                    //说明用户身份证号码输入错误，与会员本人身份证不一致
                    return new Result(false, MessageConstant.IDCARD_ORDERED);
                }
            }else {
                member = new Member();
        ...
        order.setMemberId(member.getId());  //设置会员ID值
        ...
        orderSetting.setReservations(orderSetting.getReservations() + 1); //设置一预约人数+1
